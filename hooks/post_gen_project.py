...
def add_entry_to_docs_toc(entry: str) -> None:
        ...
        for i_line, line in enumerate(buffer):
            if len(line.strip()) > 0:
                ...
                if indent < 0:
                    ...
                    raise DocsUpdateException(
                        f"Indentation error at TOC line {i_line+1}: {line_indent} spaces instead of {indent}"
                    )

        # Sanity checks
        if indent <= 0:
            raise DocsUpdateException(
                f"Indentation error in TOC: invalid indent of {indent} determined."
            )

        # Insert new line into buffer, after last non-empty line
        buffer[last_line_index] = (
            buffer[last_line_index] + indent_entry(indent) + os.linesep
        )
    ...
    try:
                    ...
                    if line_in_strip.startswith(".."):
                        if line_in_strip.lstrip(". ").startswith("toctree"):
                            if flag_processing_toc:
                                raise DocsUpdateException(
                                    f"Multiple insert points found in '{filename}.'"
                                )
        ...
        raise DocsUpdateException(
            "Could not update documentation.  File left unchanged."
        ) from e
    else:
